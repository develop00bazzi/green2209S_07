...
function joinCheck() {
    ...
    // }
        ...
        let tel1=socialJoinForm.tel1.value;
        let tel2=socialJoinForm.tel2.value;
        let tel3=socialJoinForm.tel3.value;

        // 주소 하나의 필드로 묶음 처리

        let postcode = socialJoinForm.postcode.value;
        let roadAddress = socialJoinForm.roadAddress.value;
        let detailAddress = socialJoinForm.detailAddress.value+" ";
        let extraAddress = socialJoinForm.extraAddress.value+" ";
        socialJoinForm.address.value =roadAddress+"/"+extraAddress+"/"+detailAddress+"/"+postcode;

        let tel=tel1.trim()+"-"+tel2.trim()+"-"+tel3.trim();
        socialJoinForm.phoneNum.value=tel;
...
function midOverCheck() {
    let mid=socialJoinForm.mid.value;

    if(mid.trim()=="") {
        $("#midCheck").html("<font color='red'>&nbsp;아이디를 입력해주시기 바랍니다!</font>");
        socialJoinForm.mid.focus();
        return false;
    }
...
function nickNameOverCheck() {
    let nickName = socialJoinForm.nickName.value;

    if (nickName.trim() == "") {
        $("#nickNameCheck").html("<font color='red'>&nbsp;닉네임을 입력해주시기 바랍니다!</font>");
        socialJoinForm.nickName.focus();
        return false;
...
function emailOverCheck() {
    let email = socialJoinForm.email.value;

    if (email.trim() == "") {
        $("#emailCheck").html("<font color='red'>&nbsp;이메일을 입력해주시기 바랍니다!</font>");
        socialJoinForm.email.focus();
        return false;
    }
    else if (regEmailFlag == 0) {
...
function regRePwdCheck() {
    let rePwd=socialJoinForm.rePwd.value;
    if(rePwd.trim().indexOf(" ")!=-1) {
        $("#pwdCheck").html("<font color='red'>&nbsp;비밀번호에는 공백 사용이 불가능합니다!</font>");
        regRePwdFlag=0;
        socialJoinForm.rePwd.focus();
        return false;
    }
    else if (rePwd.trim()=="") {
    ...
    else if (socialJoinForm.pwd.value!=rePwd) {
...
function regNameCheck() {
    let name=socialJoinForm.name.value;
    if(name.trim().indexOf(" ")!=-1) {
        $("#nameCheck").html("<font color='red'>&nbsp;성명은 공백 사용이 불가능합니다!</font>");
        regNameFlag=0;
        socialJoinForm.name.focus();
        return false;
    }
    else if (name.trim()=="") {
